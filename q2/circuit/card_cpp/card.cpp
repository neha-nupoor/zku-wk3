...
uint get_total_signal_no() {return 7098;}

uint get_number_of_components() {return 15;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 5286;}
...
uint csoffset = mySignalStart+7091;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "suiteEqual";
mySubcomponents[aux_create+i] = aux_cmp_num;
...
uint csoffset = mySignalStart+1781;
for (uint i = 0; i < 1; i++) {
...
uint csoffset = mySignalStart+3551;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimcHash1";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
...
uint csoffset = mySignalStart+5321;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimcHash2";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 8],&circuitConstants[1]); // line circom 22
